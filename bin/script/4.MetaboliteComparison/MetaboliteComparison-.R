...
draw_volcano = function(data,outdir,name, pvalue_cutoff=0.05, log2ratio_cutoff=1.5, pvalue_type='t.test_p.value', vip=1){
...
draw_volcano_sig_meta = function(data,outdir,name, pvalue_cutoff=0.05, log2ratio_cutoff=1.5, pvalue_type='t.test_p.value', vip=1){
...
draw_pca_score_plot = function(pca_score_file,outdir, name, ion_type){
  ...
  p_2d = ggplot(data = pca_score_file,aes(x=pca_score_file[,2],y=pca_score_file[,3],colour=class))+
    geom_point()+
    theme_bw()+
    scale_color_manual(values=getPalette(colourCount))+
  ...
  p_2d_no_qc = ggplot(data = pca_score_noqc,aes(x=pca_score_noqc[,2],y=pca_score_noqc[,3],colour=class))+
    geom_point()+
    theme_bw()+
...
for(group in groups){
  ...
  if(dim(group_data_total.significant)[1] > 0){
    ...
    system(paste("Rscript",  kegg_color_script, "--kegg_result" , paste0(group_out_dir, "/KEGG/kegg.csv"), "--kegg_dir", argv$kgml, "--de_result", paste0(group_out_dir, "/", group_new_name, ".significant.csv"), "--out_dir", paste0(group_out_dir, "/KEGG/kegg_map"), sep=" "))
